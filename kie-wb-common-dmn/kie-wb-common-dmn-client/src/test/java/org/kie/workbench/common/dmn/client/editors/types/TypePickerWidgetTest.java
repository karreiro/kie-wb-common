...
public class TypePickerWidgetTest {
    ...
    public void testSetDMNModel_Divider() {
        ...
        verify(option).setDivider(eq(true));

        final int itemsCount = BuiltInType.values().length + 1 + definitions.getItemDefinition().size();
        verify(typeSelector, times(itemsCount)).add(eq(option));
    }
