...
public class DecisionNavigatorObserver {
    ...
    private DecisionNavigatorItem getActiveParent() {
        return getTreePresenter().getActiveParent();
    }

    private void setActiveParent(final EditExpressionEvent event) {
        final String uuid = event.getNode().getUUID();
        getTreePresenter().setActiveParentUUID(uuid);
    }

    private void selectItem(final EditExpressionEvent event) {
