...
public class DecisionNavigatorNestedItemFactory {
    ...
    private Event<EditExpressionEvent> editExpressionEvent;
    ...
    boolean hasNestedElement(final Node<View, Edge> node) {
        return getOptionalHasExpression(node).isPresent() && getOptionalExpression(node).isPresent();
    }

    private EditExpressionEvent makeEditExpressionEvent(final Node<View, Edge> node) {
        ...
        return new EditExpressionEvent(currentSession, hasName, hasExpression, node);
    }

    private String getUUID(final Node<View, Edge> node) {
        final Expression expression = getExpression(node);
        return expression.getId().getValue();
    }

    private String getLabel(final Node<View, Edge> node) {
        return getExpression(node).getClass().getSimpleName();
    }

    private DecisionNavigatorItem.Type getType(final Node<View, Edge> node) {
        return ITEM_TYPE_BY_EXPRESSION.get(getExpression(node).getClass());
    }

    private HasExpression getHasExpression(final Node<View, Edge> node) {
        return getOptionalHasExpression(node).orElseThrow(RuntimeException::new);
    }

    private Optional<HasExpression> getOptionalHasExpression(final Node<View, Edge> node) {
    ...
    private Optional<Expression> getOptionalExpression(final Node<View, Edge> node) {
