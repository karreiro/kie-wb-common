...
public class DataTypeSelectView implements DataTypeSelect.View {
    ...
    HTMLOptionElement makeHTMLOptionElement() {
        // This is a workaround for an issue on Errai (related to 'ManagedInstance' + 'HTMLOptionElement').
        return (HTMLOptionElement) typeSelectOption.cloneNode(false);
    }

    HTMLOptGroupElement makeHTMLOptGroupElement() {
        // This is a workaround for an issue on Errai (related to 'ManagedInstance' + 'HTMLOptGroupElement').
    ...
    native void triggerPickerAction(final Element element,
                                    final String method) /*-{
        $wnd.jQuery(element).selectpicker(method);
    }-*/;

    native void setPickerValue(final Element element,
                               final String value) /*-{
        console.log(value);
        $wnd.jQuery(element).selectpicker('val', value);
    }-*/;

    native void setupOnChangeHandler(final Element element) /*-{
        var instance = this;
        $wnd.jQuery(element).on('hidden.bs.select', function (e) {
            instance.@org.kie.workbench.common.dmn.client.editors.types.treegrid.DataTypeSelectView::onSelectChange()();
        });
    }-*/;
}
