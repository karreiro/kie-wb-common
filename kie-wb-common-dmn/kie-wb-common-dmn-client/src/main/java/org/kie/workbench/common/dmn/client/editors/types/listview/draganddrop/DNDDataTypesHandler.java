...
public class DNDDataTypesHandler {
    ...
    public void onDropDataType(final Element currentElement,
        ...
        try {
            ...
            final DNDContext dndContext = new DNDContext(currentElement, hoverElement);
            ...
            DomGlobal.console.error(DNDDataTypesHandler.class.getName() + " error.", e.getMessage());
        }
    }

    private void shiftCurrentByReference(final DataType current,
        ...
        final DataTypeList dataTypeList = getDataTypeList();
        final DataType clone = cloneDataType(current);

        // destroy current data type
        dataTypeList.findItem(current).ifPresent(item -> item.destroy().execute());

        // create new by using shift strategy
        dataTypeList.findItemByDataTypeHash(referenceHash).ifPresent(ref -> {
            shiftStrategy.consumer.accept(ref, clone);
        });
    }

    private DataType cloneDataType(final DataType current) {
    ...
    private DNDListComponent getDndListComponent() {
        return Optional
                .ofNullable(dataTypeList.getDndListComponent())
                .orElseThrow(() -> {
    ...
    class DNDContext {
        ...
        private Optional<DataType> getCurrentDataType() {
        ...
        private Optional<DataType> getFirstDataType(final DataType current) {
            return getDataTypeList()
                    .getItems()
                    .stream()
                    .map(DataTypeListItem::getDataType)
                    .filter(dataType -> !Objects.equals(current.getName(), dataType.getName()))
                    .findFirst();
        }

        private Element getPreviousElement(final Element reference) {
            return getDndListComponent()
                    .getPreviousElement(reference, element -> {
                        return Position.getX(element) <= Position.getX(reference);
                    })
                    .orElse(null);
        }
    }
