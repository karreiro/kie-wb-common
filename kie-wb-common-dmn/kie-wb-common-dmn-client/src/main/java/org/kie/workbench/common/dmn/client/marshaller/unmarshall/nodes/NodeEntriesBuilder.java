...
class NodeEntriesBuilder {
    ...
    List<NodeEntry> buildEntries() {
        ...
        // The diagram has DMNDI elements (shapes)
        if (shapesByDiagramId.size() > 0) {
        ...
        // The diagram does not have DMNDI elements (shapes)
        if (dmnDiagrams.size() == 1 && dmnElements.size() > 0) {
            return dmnElements
                    .stream()
                    .map(dmnElement -> {
